...
const handleNav = ({ userNav, addToNav, removeFromNav,
                     ...
                     title, content, id, published, navigation }) => {
...
const handlePublish = ({ userNav, removeFromNav, togglePublish,
                         ...
                         id, published, navigation }) => {

  const index = userNav.findIndex(page => page.title === title);
  index !== -1 && published ? toggleNavPublish({ title, content, id, published, navigation }) :
                              (togglePublish({ title, content, id, published, navigation }),
                               removeFromNav({ title, content, id, published, navigation }));
}

const AdminPageCard = ({ userNav, addToNav, removeFromNav,
                         togglePublish, toggleNav, toggleNavPublish,
